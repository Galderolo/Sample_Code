...
public class Stage : MonoBehaviour
{
    ...
    public void CallBackToRaid()
    {
        ...
        switch (currentSelected)
        {
            case D1: Load_Dungeon(8, CurrenciesAndValues.keys_for_raid, CurrenciesAndValues.scrolls_for_raid, ref my_scrolls.scroll_ruby); 
                break;
            case D2: Load_Dungeon(9, CurrenciesAndValues.keys_for_raid, CurrenciesAndValues.scrolls_for_raid, ref my_scrolls.scroll_sapphire); 
                break;
            case D3: Load_Dungeon(10, CurrenciesAndValues.keys_for_raid, CurrenciesAndValues.scrolls_for_raid, ref my_scrolls.scroll_emerald); 
                break;
            case D4: Load_Dungeon(11, CurrenciesAndValues.keys_for_raid, CurrenciesAndValues.scrolls_for_raid, ref my_scrolls.scroll_topaz); 
                break;
            case D5: Load_Dungeon(12, CurrenciesAndValues.keys_for_raid, CurrenciesAndValues.scrolls_for_raid, ref my_scrolls.scroll_amethyst); 
                break;
            case D6: Load_Dungeon(13, CurrenciesAndValues.keys_for_raid, CurrenciesAndValues.scrolls_for_raid, ref my_scrolls.scroll_diamond); 
                break;
    ...
    private void SetHP(int ID)
    {
        ...
        HPFormula formula = HPFormula.Instance;

        if (ID != 8)
        {
            var tupla = formula.GetInfoDungeon1(ID);

            //HP INFO
            double _base = tupla.Item1;
            float _const = tupla.Item2;
            double start_life = formula.GetFormulaResultByWave(_base, _const, false, 1);
            double end_life = formula.GetFormulaResultByWave(_base, _const, false, 200);

            //ARMOR INFO
            double _base_armor = tupla.Item3;
            float _const_armor = tupla.Item4;
            double start_armor = formula.GetFormulaResultByWave(_base_armor, _const_armor, false, 1);
            double end_armor = formula.GetFormulaResultByWave(_base_armor, _const_armor, false, 200);

            //SHIELD INFO
            double _base_shield = tupla.Item5;
            float _const_shield = tupla.Item6;
            double start_shield = formula.GetFormulaResultByWave(_base_shield, _const_shield, false, 1);
            double end_shield = formula.GetFormulaResultByWave(_base_shield, _const_shield, false, 200);
        ...
        {
            double start_life = formula.GetFormulaResultByEscalableWave(1);
            double end_life = formula.GetFormulaResultByEscalableWave(200);
            life.text = format.formatNumber(start_life) + " - " + format.formatNumber(end_life);
